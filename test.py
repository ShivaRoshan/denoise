...
class Test(object):
    ...
    def __call__(self):
        ...
        out_m = MODEL
	#out_m = MODEL
        saverE=tf.compat.v1.train.Saver(var_list=tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='EST'))
        count_param('EST')

        vars1=tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='Denoise1')
        vars2=tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='Noise_ENC')
        saverM=tf.compat.v1.train.Saver(var_list=vars1+vars2)
